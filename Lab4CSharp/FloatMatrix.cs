...
internal class FloatMatrix
{
    ...
    public FloatMatrix(int rows, int columns, ushort value)
    {
        this.n = rows;
        this.m = columns;
        this.codeError = 0;
    ...
    ~FloatMatrix()
    {
        Console.WriteLine("FloatMatrix object is being destroyed.");
    }
public void Fill()
{
    for (int i = 0; i < n; i++)
    ...
    static public bool HasZeroElement(FloatMatrix matrix)
    {
        for (int i = 0; i < matrix.n; i++)
        {
            for (int j = 0; j < matrix.m; j++)
    ...
    public static FloatMatrix operator +(FloatMatrix matrix1, ushort scalar)
    {
        FloatMatrix matrix = new FloatMatrix(matrix1.n, matrix1.m);
        if (matrix1 == null)
        {
    ...
    public static FloatMatrix operator *(FloatMatrix left, FloatMatrix right)
    {
        ...
        FloatMatrix result = new FloatMatrix(left.n, left.m);
    ...
    {
        // Перевірка на валідність вхідних параметрів може бути додана за необхідності
        FloatMatrix result = new FloatMatrix(left.n, right.m);
        for (int i = 0; i < left.n; i++)
        {
            for (int j = 0; j < left.m; j++)
            {
    ...
    public static bool operator ==(FloatMatrix left, FloatMatrix right)
    {
